...
int monitor_process(unsigned int pid, process_state *p_state)
{
    if(pid > 0)
    {
        p_state->pcpu = get_proc_cpu(pid);
        p_state->vmrss = get_proc_mem(pid);
		p_state->vmsize = get_proc_virtualmem(pid);
    }
    return 0;
}
...
{
    unsigned int pid = get_pid(process_name);
    printf("######### [%s] -pid:[%d]- cpu:[%f]\r\n",current_process, pid,get_proc_cpu(pid));
}